 ...
 *
 * <ul>
 *   <li>{@link StompCommand#CONNECT} — JWT 검증 후 {@code AuthMember}를 Principal로 세팅</li>
 *   <li>{@link StompCommand#SUBSCRIBE} — {@code /sub/chat-rooms/{id}} 구독 시 참여자 검사</li>
 * </ul>
 *
...
public class StompAuthChannelInterceptor implements ChannelInterceptor {
	 ...
	 *
	 * <p>이후 SUBSCRIBE/SEND에서 {@code accessor.getUser()} / 컨트롤러 {@code Principal}로
	 ...
	 * 채팅방 토픽 구독 시 참여자(creator 또는 member)만 허용한다.
	 *
	 * <p>{@code /sub/chat-rooms/{chatRoomId}} 형태일 때만 검사한다.
	 ...
	 *
	 * <p>예: {@code /sub/chat-rooms/42} → {@code 42L}
	 *
	 * @throws CustomException ID 없음·형식 오류·숫자 아님 → {@link ChatErrorCode#CHAT_ROOM_NOT_FOUND}
	 */
	 ...
	 *
	 * <p>CONNECT 시 {@link #handleConnect}가 {@link UsernamePasswordAuthenticationToken}으로
	 * principal에 {@code AuthMember}를 넣어 둔 것을 기대한다.
	 *
	 * @throws CustomException CONNECT 전이거나 인증 정보 없음 → {@link ChatErrorCode#CHAT_LOGIN_REQUIRED}
	 */
	 ...
	 *
	 * <p>클라이언트 환경마다 토큰 전달 방식이 달라 우선순위로 찾는다.
