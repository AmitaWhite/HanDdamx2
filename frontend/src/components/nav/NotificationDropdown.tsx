			...
			{open && (
				<div className="absolute right-0 top-full z-50 mt-2 w-80 overflow-hidden rounded-xl border border-outline-variant/30 bg-surface-container-lowest shadow-card-hover">
					<div className="flex items-center justify-between border-b border-outline-variant/50 px-4 py-3">
						<h2 className="text-label-md font-label-md font-bold text-on-surface">알림</h2>
						<button
							type="button"
							onClick={markAllRead}
					...
					<div className="max-h-96 divide-y divide-outline-variant/30 overflow-y-auto">
						{preview.map((n) => (
							<Link
								...
								className={cn(
									"flex items-start gap-3 px-4 py-3 transition-colors hover:bg-surface-container-low",
									!n.isRead && "bg-primary/5",
								)}
							>
								<span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-surface-container text-secondary">
									<Icon name={NOTIFICATION_TYPE_ICON[n.type]} className="text-[18px]" />
								</span>
								<div className="min-w-0 flex-1">
									<p className={cn("text-caption font-caption text-on-surface", !n.isRead && "font-bold")}>
										{n.message}
										{n.amount != null && (
											<span className="ml-1 font-bold text-primary">{n.amount.toLocaleString()}원</span>
										)}
									</p>
									<p className={cn("mt-0.5 text-[11px]", !n.isRead ? "text-primary" : "text-secondary")}>
										{n.createdAtLabel}
									</p>
								</div>
							</Link>
						...
						{preview.length === 0 && (
							<p className="px-4 py-8 text-center text-caption font-caption text-secondary">알림이 없어요.</p>
						)}
					</div>
					<Link
