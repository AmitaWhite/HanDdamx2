...
export function DashboardPostsPage() {
	...
	const myPosts = mockPosts.filter((p) => p.creatorId === CURRENT_CREATOR_ID);
	...
	return (
		<div className="container-page py-8">
			...
			<section className="mb-12">
				<div className="mb-4 flex items-center gap-4">
					{(["recent", "popular"] as const).map((s) => (
						<button
							...
							type="button"
							onClick={() => setSort(s)}
							className={"text-label-md font-label-md " + (sort === s ? "font-bold text-primary" : "text-secondary")}
						>
							{s === "recent" ? "최신순" : "인기순"}
						</button>
					))}
				...
				{sorted.length === 0 && <p className="py-8 text-center text-body-md text-secondary">작성한 게시물이 없어요.</p>}
			</section>
